...
5. **Worktree launch + apply gate (default).**
   ...
   - **Delegate to the worktree runtime.** Capture the JSON report to a temp file; do not feed untrusted patch text through stdin or a heredoc later.
     ...
     ```bash
     REPORTF=$(mktemp)
     python "${CLAUDE_PLUGIN_ROOT}/scripts/ollama_agent.py" --as-claude --repo "<cwd>" --task-file "$TASKF" --model "<validated-model>" --timeout <sec> --permission-mode <mode> [--max-iters <N>] > "$REPORTF"
     ```
...
6. **Direct launch (`--no-worktree`, `--resume`, or non-git cwd fallback).** The parsed `--timeout <sec>` (default `1800`) bounds this path via coreutils `timeout`; the launch block below resolves the binary (`timeout`, else macOS/Homebrew's `gtimeout`) and, if neither is installed, runs uncapped and says so rather than failing. Note: the calling Bash tool's own maximum is `600s`, so a run longer than that must use `run_in_background` or a sub-600s `--timeout`.
   - **Launch disclosure — state it, do not ask.** In one line before launching: full Claude Code agent (all tools except `Agent`/`Task`, plus skills, MCP, hooks) on `<model>`, permission mode `<mode>`, running on your **real** tree with no isolation and no diff review — changes land directly, and at `bypassPermissions` that is arbitrary code execution on this host. A cloud model means everything it reads and does is exposed to ollama.com.
   - **Mint task file.** `TASKF=$(python -c "import tempfile,os; print((os.path.join(tempfile.mkdtemp(),'task.md')).replace(os.sep,'/'))")` (use `py -3` if `python` is missing). Use the `Write` tool to put the task text into `$TASKF`.
   - **Launch and capture the stream.** `<permission-flags>` is **exactly one of these six literals, copied verbatim** — never build it by interpolating the user's text, so nothing from `$ARGUMENTS` reaches the shell: `--dangerously-skip-permissions` (for `bypassPermissions`, the default) · `--permission-mode acceptEdits` · `--permission-mode auto` · `--permission-mode default` · `--permission-mode dontAsk` · `--permission-mode plan`. If step 2's value is not one of the six modes, stop — do not pass it through. Never emit two of these literals at once: `--dangerously-skip-permissions` wins and the mode the user asked for is silently ignored. Include `--resume` only if step 3 validated one. `-p` requires `--verbose` alongside `--output-format stream-json`. Tee the raw stream to a capture file next to `$TASKF` so a `timeout` kill still leaves the events written so far on disk — a single terminal JSON object (the old `--output-format json`) produces nothing at all when killed mid-run. The `$WRAP` prefix below applies the cap from step 6's header; when no timeout binary exists it is empty and word-splits to nothing, leaving the launch unbounded. Include `[--max-turns <N>]` only when step 2 parsed `--max-iters <N>` (map the value verbatim); omit it otherwise so the run is not turn-capped:
     ```bash
     ...
     ```
     Then parse the capture file with a surrogate-safe UTF-8 parser: `session_id` comes from the first line that carries one (the `system`/`init` event, emitted before anything else, regardless of which line ends up as the result), while `is_error`/`total_cost_usd`/`result` come from the last line whose `type` is `result`. If no `result` line was captured (killed or crashed before claude finished), report that explicitly instead of printing nothing — still with the same four keys, `session_id` still filled in from the init line when one was captured:
     ```bash
     python -c "import sys,json; exec('def _p(l):\n try: return json.loads(l)\n except Exception: return None\n'); path,ec=sys.argv[1],int(sys.argv[2]); evs=[e for e in (_p(l) for l in open(path,encoding='utf-8',errors='replace') if l.strip()) if e is not None]; sid=next((e.get('session_id') for e in evs if e.get('session_id')), None); res=[e for e in evs if e.get('type')=='result']; r=res[-1] if res else None; out={k:r.get(k) for k in ('is_error','session_id','total_cost_usd','result')} if r else {'is_error':True,'session_id':None,'total_cost_usd':None,'result':'no final result: the launch ended before claude emitted one (exit %d)'%ec}; out['session_id']=sid; sys.stdout.reconfigure(encoding='utf-8',errors='replace'); print(json.dumps(out, ensure_ascii=False))" "$CAPF" "$LAUNCH_EXIT"
     ```
   - **Report.**
     - Relay the JSON's `result` verbatim — that is the session's final answer / account of what it did (or, on a missing `result` line, the synthesized "no final result" message).
     ...
     - If the launch did NOT exit cleanly (a `timeout`/`gtimeout` kill, a non-zero exit, or a capture file with no `result` line), the direct path has no diff capture and edits the **real** tree — so partial edits may be sitting in your working tree with no JSON to describe them. Give the caller `$CAPF`'s path (it survives the cleanup step below only until then) and, if a `session_id` was recovered from the init line, say it can be resumed with `/ollama:as-claude --resume <session_id>`. Run `git status --short` in the real repo and surface the result, leading with the fact that the session may have left partial edits. (This path has no worktree — do NOT copy the leaked-worktree recovery from step 5.)
   - Remove the task directory only after the report above is printed (including on an error stop), via the already-allowed Python tool: `python -c "import os,sys,shutil; shutil.rmtree(os.path.dirname(sys.argv[1]), ignore_errors=True)" "$TASKF"`.
