...
- Send the question on stdin via a heredoc. Choose a terminator token that does **not** appear anywhere in the question (a lone matching line would end the heredoc early); `OLLAMA_CC_EOF` is a safe default. Use `py -3` if `python` is missing. Preserve `--model` if the user gave one; otherwise the runtime uses its configured default:

```bash
python "${CLAUDE_PLUGIN_ROOT}/scripts/ollama_companion.py" run [--model <name>] <<'OLLAMA_CC_EOF'
<the user's question>
OLLAMA_CC_EOF
```
