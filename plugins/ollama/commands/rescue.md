...
argument-hint: '[--model <name>] [--allow-shell] [--timeout <sec>] [--max-iters <N>] <what the ollama agent should do>'
...
5. **Check `stop_reason` on every run before reading the diff or the answer** — a non-`done` stop returns an empty `final` indistinguishable from "nothing to report". `egress_budget` / `api_error` / `timeout` stops are excluded from synthesis **by design** (no `final` is salvaged from them); do not "fix" this. Present the report's `final` summary and its `diff`. If `stop_reason` is not `done`, or the `diff` is empty, say so and stop — there is nothing to apply. **If `partial_work` is true, lead with it**: the run stopped early *after* a successful write or shell command, so an empty or thin `diff` does NOT mean it did nothing. If `written_paths` is non-empty, name those files — a dead agent's own scratch script is often the only record of what it was mid-way through. ⚠ `written_paths` records successful **`write_file` calls only**: a file created by a shell command does not appear in it, so the list is a floor, never the full set of what changed. It can therefore be empty while `partial_work` is true (the run only got as far as shell commands) — say that, rather than reporting "nothing was written". (`partial_work` only fires on a write run; the read-only consult path `--root --readonly` raises no `partial_work` at all.) ⚠ Do not tell the user to go open those paths — they are worktree-relative and the worktree is already removed. The captured `diff`/`diff_file` is the surviving record; the paths tell them *what* to look for in it. If `cleanup_error` is present the worktree survived, and only then is it inspectable at the leaked path — mention it.
