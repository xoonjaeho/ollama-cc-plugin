...
5. Present the report's `final` summary and its `diff`. If `stop_reason` is not `done`, or the `diff` is empty, say so and stop — there is nothing to apply. **If `partial_work` is true, lead with it**: the run stopped early *after* a successful write or shell command, so an empty or thin `diff` does NOT mean it did nothing. If `written_paths` is non-empty, name those files — that is what the agent wrote, and a dead agent's own scratch script is often the only record of what it was mid-way through. `written_paths` lists **file writes only**, so it can be empty while `partial_work` is true (the run only got as far as shell commands); say so rather than reporting "nothing was written". ⚠ Do not tell the user to go open those paths — they are worktree-relative and the worktree is already removed. The captured `diff`/`diff_file` is the surviving record; the paths tell them *what* to look for in it. If `cleanup_error` is present the worktree survived, and only then is it inspectable at the leaked path — mention it.
